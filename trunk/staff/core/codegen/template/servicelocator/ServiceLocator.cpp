...
#ifeqend
      if (sName == "$(Class.ServiceName)")
        return new $($sNamespace)$(Class.Name)Proxy(pService, sID);
#end
#end
...
#var sNamespacePrefix 
      ...
      if (sName == "$(Class.ServiceName)")
      {
        reinterpret_cast<$($sNamespace)$(Class.Name)Proxy*>(pProxy)->SetService(pService);
        return;
      }
#end
#end
...
#var sNamespacePrefix 
          ...
          if (sName == "$(Class.ServiceName)")
          {
            delete reinterpret_cast<$($sNamespace)$(Class.Name)Proxy*>(it->second);
            continue;
          }
#end
#end
