...
{
public:
  static void Find(const std::string& sInDir, std::list<std::string>& rList, const std::string& sMask = "*.*")
  {
    rList.clear();
#ifdef WIN32
    intptr_t nFile; 
    ...
    if( nFile != -1 )
      do
      {
        if((stSearchData.attrib & _A_SUBDIR) == 0)
    ...
    if(pDir != NULL)
    {
      ...
      while((pstDirent = readdir(pDir)) != NULL)
      {
        ...
        if(((stStat.st_mode & S_IFDIR ) == 0) && (fnmatch(sMask.c_str(), pstDirent->d_name, 0) == 0))
    ...
    {
      std::cout << "can't open dir " << sInDir << ": " << strerror(errno) << std::endl;
    }
#endif
  }
};
...
public:
  ...
  void Init(const std::string& sInDir)
  {
    ...
    CFileFind::Find(sInDir, tFileList, "*.*");
