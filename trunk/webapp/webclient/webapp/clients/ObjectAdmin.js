...
{
  var tResult = {};
  tResult.nId = parseInt(tOperation.SubNodeText('nId', tNode));
  tResult.sName = tOperation.SubNodeText('sName', tNode);
  tResult.sDescription = tOperation.SubNodeText('sDescription', tNode);
  tResult.nParentId = parseInt(tOperation.SubNodeText('nParentId', tNode));
  return tResult;
}
...
staff.admin.ObjectAdmin.prototype =
{
  initialize: function(sServiceUri, sSessionId, sInstanceId, sTargetNamespace)
  {
    if (!sServiceUri)
    {
      sServiceUri = webapp.Env.protocol + webapp.Session.sHost + (webapp.Session.sPort ? (':' + webapp.Session.sPort) : '') + '/axis2/services/staff.admin.ObjectAdmin';
    }
