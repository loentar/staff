...
function DeserializeStruct_webapp_admin_SProfile(tOperation, tNode)
{
  ...
  tResult.sName = tOperation.SubNodeText('sName', tNode);
  tResult.bIsAdmin = tOperation.SubNodeText('bIsAdmin', tNode).toLowerCase() in {"1":1, "true":1};
...
webapp.admin.ProfileAdmin.prototype =
{
  initialize: function(sServiceUri, sSessionId, sInstanceId, sTargetNamespace)
  {
    if (!sServiceUri)
    {
      sServiceUri = webapp.Env.protocol + webapp.Session.sHost + (webapp.Session.sPort ? (':' + webapp.Session.sPort) : '') + '/axis2/services/webapp.admin.ProfileAdmin';
    }
